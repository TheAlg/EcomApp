class Wizard {
    ...
    constructor(args) {
        ...
        let opts = {
            ...
            "wz_button_style": ".btn .btn-primary",
            "wz_step": ".wizard-step",
            "wz_form": ".wizard-form",
            "wz_next": ".next",
            "wz_prev": ".prev",
    ...
    check(currentStep) {
        try {
            ...
            this.current_step = currentStep //setting current step before setting content
            this.set(wz_nav_steps, wz_content_steps, currentStep) //setting content
            //console.log(wz_nav_steps)
            //console.log(wz_content_steps)
    ...
    set($wz_nav, $wz_content, active_index = 0) {
        //var active = false;

        for (let i = 0; i < $wz_nav.length; i++) {

            //active = (active === false) ? $_.hasClass($wz_nav[i], "active") : active;
            //active_index = ($_.hasClass($wz_nav[i], "active")) ? i : active_index;

            $wz_nav[i].setAttribute("data-step", i);
    ...
    checkButtons(next=null, prev=null) {
        if (next === null || prev === null){
            let wz = $_.getSelector(this.wz_class);
            let buttons = $_.getSelector(this.wz_buttons, wz);
            next = $_.getSelector(this.wz_button + this.wz_next, buttons);
            prev = $_.getSelector(this.wz_button + this.wz_prev, buttons);
        }
    ...
    refresh(){
        let buttons = $_.getSelector(this.wz_buttons);
        let prev = buttons.children[0];
        let next = buttons.children[1];

        this.checkButtons(next, prev)

        let nav = $_.getSelector(this.wz_nav);
        let content = $_.getSelector(this.wz_content);
    ...
    getFormData(){
        let content = $_.getSelector(this.wz_content).children;
        var formData;
        var action;
        for (let i=0; i<content.length; i++){
            if (content[i].classList.contains('active')){
                formData = new FormData(content[i].children[0]);
                action = content[i].children[0].getAttribute('action');
            }
        }
        return {
    ...
    forward(){
        this.refresh();
        this.locked = true;
        let nextStep = this.current_step + 1
